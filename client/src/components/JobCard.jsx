    ...
    return (
        <div className='border border-gray-200 p-6 shadow rounded'>
            ...
            <div className='mt-4 flex gap-4 text-sm'>
                <button onClick={() => navigate(`/apply-job/${job._id}`)} className='bg-blue-600 text-white px-4 py-2 rounded cursor-pointer'>Apply Now</button>
                <button onClick={() => navigate(`/apply-job/${job._id}`)} className='text-gray-500 border border-gray-300 rounded px-4 py-2 cursor-pointer'>Read More</button>
            </div>
        </div>
    )
}
